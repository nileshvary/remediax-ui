...
import { ArrowRight, Sparkles, AlertTriangle, AlertCircle, Shield } from 'lucide-react';

const API = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:8001';

const SEV_CONF: Record<string, number> = { CRITICAL: 95, HIGH: 82, MEDIUM: 65, LOW: 45 };
const SEV_COLOR: Record<string, string> = { CRITICAL: '#FF2D55', HIGH: '#FF8C00', MEDIUM: '#EAB308', LOW: '#00D4FF' };
const SEV_ICON: Record<string, React.ElementType> = { CRITICAL: AlertTriangle, HIGH: AlertTriangle, MEDIUM: AlertCircle, LOW: Shield };
...
const FALLBACK: Insight[] = [
  { id: 1, category: '—', title: 'No scan data', message: 'Run a scan to see LLM vulnerability insights here.', confidence: 0, severity: 'LOW' },
  { id: 2, category: '—', title: 'No scan data', message: 'Run a scan to see LLM vulnerability insights here.', confidence: 0, severity: 'LOW' },
  { id: 3, category: '—', title: 'No scan data', message: 'Run a scan to see LLM vulnerability insights here.', confidence: 0, severity: 'LOW' },
];
    ...
    <div className="relative w-16 h-16 flex-shrink-0 animate-brain-pulse">
      <svg viewBox="0 0 64 64" className="w-full h-full">
        ...
        <g transform="translate(10, 12)" filter="url(#brainFilter)">
          <path d="M22,8 C14,6 6,10 5,18 C4,24 7,28 10,30 C8,32 7,36 9,39 C11,42 15,42 18,40 L22,40 L22,8 Z"
            fill="none" stroke="#A78BFA" strokeWidth="1.5" strokeLinecap="round" />
          <path d="M22,8 C30,6 38,10 39,18 C40,24 37,28 34,30 C36,32 37,36 35,39 C33,42 29,42 26,40 L22,40 L22,8 Z"
            fill="none" stroke="#A78BFA" strokeWidth="1.5" strokeLinecap="round" />
          ...
          {[[10,20],[22,22],[34,22],[8,26],[22,28],[36,26],[10,32],[22,34],[34,32]].map(([x,y],i) => (
            <circle key={i} cx={x} cy={y} r="1.5" fill="#A78BFA"
              style={{ animation: `blink ${1+i*0.2}s ease-in-out infinite`, animationDelay: `${i*0.15}s` }} />
          ))}
        </g>
      </svg>
      <div className="absolute inset-0" style={{ animation: 'rotate-slow 4s linear infinite' }}>
        <div className="absolute w-2 h-2 rounded-full"
          style={{ top: '4px', left: '50%', transform: 'translateX(-50%)', background: '#A78BFA', boxShadow: '0 0 8px #A78BFA' }} />
      </div>
    </div>
  );
}
...
export default function AIInsights() {
  ...
  useEffect(() => {
    fetch(`${API}/api/findings`)
      ...
      .then((findings: any[]) => {
        ...
        const top3 = findings
          .filter(f => f.is_successful_attack)
          .sort((a, b) => (SEV_ORDER[a.severity?.toUpperCase() || 'LOW'] ?? 3) - (SEV_ORDER[b.severity?.toUpperCase() || 'LOW'] ?? 3))
          .slice(0, 3);
        if (top3.length === 0) return;
        setInsights(top3.map((f, i) => {
          const sev = (f.severity || 'LOW').toUpperCase();
          const prompt = (f.attack_prompt || '').slice(0, 120);
          return {
            id: i + 1,
            category: f.owasp_llm_category || '—',
            title: probeName(f.probe_name || 'Unknown Probe'),
            message: prompt ? `Attack prompt: "${prompt}${prompt.length === 120 ? '…' : ''}"` : 'Successful attack detected.',
            confidence: SEV_CONF[sev] ?? 50,
            severity: sev,
          };
        }));
      })
  ...
  const SevIcon = SEV_ICON[insight.severity] || AlertCircle;
  const sevColor = SEV_COLOR[insight.severity] || '#00D4FF';
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold ml-auto px-1.5 py-0.5 rounded"
              style={{ background: 'rgba(139,92,246,0.15)', color: '#A78BFA', fontSize: '10px' }}>
              {insight.confidence}% confidence
            </span>
          </div>
          <p className="text-xs font-semibold mb-1" style={{ color: sevColor }}>{insight.title}</p>
      ...
      <div>
        ...
        <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
          <div className="h-full rounded-full transition-all duration-1000"
            style={{ width: `${insight.confidence}%`, background: 'linear-gradient(90deg, #7C3AED, #A78BFA)', boxShadow: '0 0 8px rgba(167,139,250,0.5)' }} />
        </div>
      </div>
