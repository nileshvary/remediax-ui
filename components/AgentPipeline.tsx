...
export default function AgentPipeline() {
  ...
  return (
    <div
      ...
      style={{
        background: '#0D1117', border: '1px solid #1e293b', borderRadius: 10, overflow: 'hidden',
        animation: 'fade-in-up 0.5s ease-out 0.15s both',
      }}
    >
      {/* Header */}
      <div className="flex items-start justify-between px-5 py-3 flex-shrink-0" style={{ borderBottom: '1px solid #1e293b' }}>
        <div>
          <div className="flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full flex-shrink-0 inline-block" style={{ background: '#22C55E', animation: 'blink 2s ease-in-out infinite' }} />
            <span style={{ color: '#F1F5F9', fontSize: 16, fontWeight: 700 }}>RemediAX Agent Pipeline</span>
          </div>
          <p className="mt-0.5 ml-3.5" style={{ color: '#475569', fontSize: 11 }}>Real-time AI security remediation flow</p>
        </div>
      ...
      <div className="relative w-full" style={{ paddingBottom: svgPad, background: '#0D1117' }}>
        ...
        <svg viewBox={`0 0 ${VBW} ${VBH}`} className="absolute inset-0 w-full h-full" style={{ display: 'block', overflow: 'visible' }}>
          <defs>
          ...
          {[144, 312, 468, 636].map(x => (
            <line key={x} x1={x} y1="6" x2={x} y2={VBH-6} stroke="#1e293b" strokeWidth="1" strokeDasharray="3,7" strokeOpacity="0.8" />
          ))}
          {CONNECTIONS.map(c => (
            <path key={`line-${c.id}`} d={c.d} fill="none" stroke={c.color} strokeWidth="1" strokeDasharray="5,4" strokeOpacity="0.38" />
          ))}
          {CONNECTIONS.flatMap(c =>
            Array.from({ length: c.dots }, (_, k) => (
              <circle key={`dot-${c.id}-${k}`} r="2" fill={c.color} fillOpacity="0.92">
                <animateMotion dur="2s" begin={`${(-(k * 2 / c.dots)).toFixed(3)}s`} repeatCount="indefinite">
                  <mpath href={`#${c.id}`} />
                </animateMotion>
              </circle>
            ))
          ...
          {ARROWS.map(([pts, col], i) => (
            <polyline key={i} points={pts} fill="none" stroke={col} strokeWidth="1.1" strokeLinecap="round" strokeLinejoin="round" strokeOpacity="0.75" />
          ))}
        </svg>

        {/* HTML card layer — CSS entrance + framer-motion hover */}
        {CARDS.map((c, i) => {
          const Icon = c.Icon;
          const isHub = c.id === 'a2';
          return (
            <motion.div
              ...
              style={{
                ...
                background: '#0f1523',
                border: `${isHub ? 1.5 : 1}px solid ${c.color}${isHub ? 'cc' : '66'}`,
                borderRadius: 8,
                boxShadow: `0 0 0 1px ${c.color}${isHub ? '40' : '20'}, inset 0 0 14px ${c.color}0a`,
                animation: `fade-in-up 0.4s ease-out ${0.3 + i * 0.12}s both`,
              }}
              whileHover={{
                scale: 1.07,
                boxShadow: `0 0 0 1.5px ${c.color}cc, 0 4px 20px ${c.color}35`,
                transition: { duration: 0.15 },
              }}
            >
              <Icon size={12} style={{ color: c.color, opacity: 0.9, flexShrink: 0 }} />
              <span style={{ color: '#94A3B8', fontSize: 9,  fontWeight: 500, letterSpacing: '0.04em', lineHeight: 1.1, fontFamily: 'Inter, ui-sans-serif, sans-serif' }}>
                {c.title}
              </span>
              <span style={{ color: '#F1F5F9', fontSize: 10, fontWeight: 700, lineHeight: 1.1, fontFamily: 'Inter, ui-sans-serif, sans-serif' }}>
                {c.name}
              </span>
              <span style={{ color: c.color, fontSize: 8, opacity: 0.85, lineHeight: 1, fontFamily: 'Inter, ui-sans-serif, sans-serif', background: `${c.color}15`, border: `0.5px solid ${c.color}40`, borderRadius: 3, padding: '1px 5px', marginTop: 1 }}>
                {c.tool}
              </span>
            </motion.div>
          );
        ...
        <div className="flex items-start gap-4 px-5 pt-4 pb-2">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-start justify-between gap-4">
              ...
              <div className="text-right flex-shrink-0" style={{ color: '#334155', fontSize: 10.5, lineHeight: 1.7 }}>NVD API · MITRE ATLAS<br />OWASP · Garak probes</div>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-5 flex-shrink-0" style={{ borderTop: '1px solid #1e293b' }}>
        {STATS.map((s, i) => (
          <div key={s.label} className="flex flex-col items-center justify-center py-3" style={{ borderRight: i < STATS.length-1 ? '1px solid #1e293b' : 'none' }}>
